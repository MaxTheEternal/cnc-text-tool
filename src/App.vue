    ...
    <div class="page">
      ...
      <div class="sidebar">
        <div class="controls-wrapper">
          <div class="controls">
            <p class="mt-3 lead text-center text-white">Schleifen Designer</p>
            ...
            <transition name="slide">
              <div v-if="source.svg">
                ...
                <double-text-input
                  ...
                  first-label="x:"
                  second-label="y:"
                  :first-value="overlay.x"
                  :second-value="overlay.y"
                  @firstInput="moveOverlayX"
      ...
      <div class="paper">
        <div id="sketch" class="sketch">
          <div id="svg-wrapper" ref="svgWrapper"></div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      canvas: {
        widthCm: 20, // Default A4 width in cm
        heightCm: 25, // Default A4 height in cm
      },
  ...
  mounted() {
    ...
    this.fontOptions = Object.keys(this.fonts)
      .sort()
      .map((fontName) => {
    ...
    this.$nextTick(() => {
      this.processInputSVG(
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 612 792"><title>CNC Fill Text</title></svg>'
      );
    });
  ...
  methods: {
    updateCanvasSize() {
      ...
      const widthPx = this.canvas.widthCm * CM_TO_PX;
      const heightPx = this.canvas.heightCm * CM_TO_PX;

      this.overlay.svg.size(widthPx, heightPx);
      this.overlay.svg.viewbox(0, 0, widthPx, heightPx);

      if (this.source.svg) {
        this.source.svg.setAttribute("width", widthPx);
        this.source.svg.setAttribute("height", heightPx);
      }

      this.overlay.width = widthPx;
      this.overlay.height = heightPx;
      this.source.width = widthPx;
      this.source.height = heightPx;
    },
    ...
    async processInputSVG(fileContents) {
      ...
      this.source.viewbox = svgElement.hasAttribute("viewBox")
        ? svgElement.getAttribute("viewBox")
        : false;
      this.source.width = svgElement.hasAttribute("width")
        ? svgElement.getAttribute("width")
        : 0;
      this.source.height = svgElement.hasAttribute("height")
      ...
      if (this.source.viewbox) {
        ...
        const viewBoxDelimeter =
          this.source.viewbox.indexOf(",") > -1 ? "," : " ";
        const parsedViewbox = this.source.viewbox
          .split(viewBoxDelimeter)
          .map((value) => parseFloat(value));
        this.overlay.svg.viewbox(
          parsedViewbox[0],
          parsedViewbox[1],
          parsedViewbox[2],
          parsedViewbox[3]
      ...
      // Retrieve width Unit
      const widthUnitPresent =
        typeof this.source.width == "number"
          ? false
          : this.source.width.match(/[a-zA-Z]+/g);
    ...
    async loadFont() {
      ...
      if (!fontMeta.data) {
        ...
        fontMeta.string = await this.loadFontFromURL(
          `${process.env.BASE_URL}fonts/${fontMeta.filename}.svg`
        );
      }
      fontMeta.data = parseFont(
        new DOMParser().parseFromString(fontMeta.string, "image/svg+xml"),
        this.font.size
      );
    ...
    download() {
      ...
      downloadSVG(
        this.$refs.svgWrapper.childNodes[1],
        flattenedGroup,
        `line-text-${Date.now()}`
      );
    },
        ...
        if (fontData[character]) {
          if (fontData[character].d) {
            ...
            if (this.font.alignment === "center")
              characterXPosition = originX - lineWidths[lineIndex] / 2;
            if (this.font.alignment === "right")
              characterXPosition =
                originX - maxLineWidth - lineWidths[lineIndex];
            this.paths.push({
